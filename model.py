...
class SoftPromptGenerator(nn.Module):
    ...
    def forward(self, hidden_states=None, seq_lengths=0):
        # H[cls]
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def add_prompt_generator(self):
        self.prompt_generator = SoftPromptGenerator(self.config)
    ...
    def forward(
        ...
        if attention_mask is not None:
            prompt_attention_mask = torch.ones(batch_size, self.num_prompt_tokens).to(device)
        ...
        output_shape = (batch_size, self.num_prompt_tokens + input_ids.size(1), hidden_states.size(-1))
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if i + 1 == self.add_prompt_layer:
                ...
                prompt_embedding = self.prompt_generator(hidden_states, seq_lengths=0)
                hidden_states = torch.cat([prompt_embedding, hidden_states], dim=1)
                if attention_mask is not None:
                    attention_mask = torch.cat([prompt_attention_mask, attention_mask], dim=-1)
